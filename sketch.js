...
// Adds a centered title "RAINDROPS" above "CLICK TO START"
...
function setup(){
  ...
  textAlign(CENTER,CENTER); // center for title + hint
...
function draw(){
  ...
  if (hintAlpha>0){
    ...
    // Title and hint, both centered vertically around screen center
    push();
    textAlign(CENTER, CENTER);
    textSize(28); // title larger
    text("RAINDROPS", width/2, height/2 - 22);
    textSize(21); // hint
    text(HINT_TEXT,    width/2, height/2 + 12);
    pop();
  }
}
function spawnDrop(){ const x=random(width), y=random(height); drops.push(new Drop(x,y)); if (ENABLE_SOUND && audioEnabled && synth){ const midi=random(G_MAJOR_MIDI); const hz=midiToFreq(midi); playPlop(hz); } }
...
function currentIntervalMs(){ const now=millis()/1000; let e=now-seg.t0; if (e>=seg.dur){ const endVal=seg.endMs; if (seg._presetHoldNext){ seg=makeHoldSegment(FAST_INTERVAL_MS,45); } else { const dur=randomChoice(RANDOM_DURATIONS_SEC); const target=randomIntervalMsStepped(MIN_INTERVAL_MS,MAX_INTERVAL_MS,INTERVAL_STEP_MS,endVal); seg=makeRampSegment(endVal,target,dur); } e=0; } if (seg.type==="hold") return seg.startMs; const t=easeInOutCubic((now-seg.t0)/seg.dur); return lerp(seg.startMs, seg.endMs, t); }
function randomChoice(arr){ return arr[floor(random(arr.length))]; }
function randomIntervalMsStepped(minMs,maxMs,step,currentVal){ const steps=floor((maxMs-minMs)/step)+1; if(steps<=1) return minMs; let idx=floor(random(steps)); let val=minMs+idx*step; if(val===currentVal){ idx=(idx+1)%steps; val=minMs+idx*step; } return val; }
function playPlop(hz){ synth.play(hz, NOTE_VELOCITY, 0, NOTE_LEN_SECONDS); const drop=random(3,7); const target=hz*pow(2,-drop/12); synth.oscillator.freq(target,0.05); }
let _startAttempted=false; async function startIfNeeded(){ if(_startAttempted) return; _startAttempted=true; started=true; hintFading=true; nextSpawnDueMs=millis()+10; scheduleNext(SLOW_INTERVAL_MS); try{ const ctx=getAudioContext(); await userStartAudio(); if(ctx.state!=='running') await ctx.resume(); audioEnabled=(ctx.state==='running'); } catch(e){ audioEnabled=false; } }
